...
public class ReversedResultTest {
	...
	public void testReverseStringWith4Words() throws Exception {
		...
		assertEquals("software cloud space rack", result.getReversed());
		
	}
	
	@Test
	public void testReverseStringWith2Words() throws Exception {
		setUp();
		String uri = "/api/rack space";
		MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.get(uri).accept(MediaType.APPLICATION_JSON_VALUE))
				.andReturn();
		...
		assertEquals(status,200);
		ReversedResult result = mapFromJson(content, ReversedResult.class);
		assertEquals("space rack", result.getReversed());
